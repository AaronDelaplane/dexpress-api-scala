...
import common.{Asset, ErrorOr, SteamAsset, SteamDescription, SteamInventory, SteamTag}
...
object ToAssets {
  ...
  def toAssets(refreshId: UUID, steamId: String, steamInventory: SteamInventory): IO[NonEmptyList[Asset]] =
    for {
  ...
  private def buildAssets(refreshId: UUID, steamId: String, xs: (NonEmptyList[SteamAsset], NonEmptyList[SteamDescription])): ErrorOr[NonEmptyList[Asset]] = {
    ...
    (NonEmptyList.fromList[String](errors), NonEmptyList.fromList[Asset](assets)) match {
      case (None, None)            => "no-errors-or-assets-generated".asLeft[NonEmptyList[Asset]]
      case (Some(errors), Some(_)) => s"errors-&-assets-generated: $errors".asLeft[NonEmptyList[Asset]]
      case (Some(errors), None)    => s"only-errors-generated: $errors".asLeft[NonEmptyList[Asset]]
  ...
  private def buildAsset(refreshId: UUID, steamId: String, a: SteamAsset)(d: SteamDescription): ErrorOr[Asset] =
    for {
